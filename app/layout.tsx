...
export const metadata: Metadata = {
  ...
  description:
    "Cybersecurity software, services, pricing, compliance readiness, and trust-center resources from GEM Cybersecurity Assist.",
  robots: {
    index: true,
    follow: true,
    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/90 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/checkout"
            className="hidden rounded-full border border-white/15 px-4 py-2 text-sm font-bold text-white hover:bg-white/10 sm:inline-flex"
          >
            Checkout
          </Link>
          <Link
            href="/store"
            className="rounded-full bg-emerald-300 px-4 py-2 text-sm font-black text-slate-950 hover:bg-emerald-200"
          >
            Shop software
          </Link>
        </div>
  ...
  return (
    <footer className="border-t border-white/10 bg-slate-950 text-slate-400">
      <div className="border-b border-white/10 bg-white/[0.03]">
        <div className="mx-auto grid max-w-7xl gap-4 px-4 py-8 sm:px-6 md:grid-cols-3 lg:grid-cols-6 lg:px-8">
          {trustSignals.map((signal) => (
            <div
              key={signal}
              className="rounded-2xl border border-white/10 bg-slate-900/60 p-4 text-center text-xs font-bold text-slate-200"
            >
        ...
        <p className="mx-auto max-w-7xl px-4 pb-6 text-center text-xs text-slate-500 sm:px-6 lg:px-8">
          Trustpilot, Google, certification, and approval marks should be
          activated only after verified third-party accounts or formal
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 md:grid-cols-5 lg:px-8">
        ...
        <div>
          ...
          <div className="mt-3 grid gap-2 text-sm">
            ...
            <Link href="/merchant-feed.txt" className="hover:text-white">
              Merchant feed
            </Link>
          </div>
        </div>
      ...
      <div className="border-t border-white/10 px-4 py-5 text-center text-xs text-slate-500">
        © {new Date().getFullYear()} {company.name}. Business address:{" "}
        {company.address}.
      </div>
    </footer>
  );
