...
export const metadata = {
  title: "Checkout",
  description:
    "Checkout handoff for GEM Cybersecurity Assist annual software subscriptions.",
};
...
export default async function CheckoutPage(props: {
  ...
  const selected = searchParams?.product
    ? getProductBySlug(searchParams.product)
    : products[0];
    ...
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="mb-10 max-w-3xl">
        ...
        <h1 className="mt-5 text-5xl font-black tracking-[-0.06em] text-white">
          Request annual software access.
        </h1>
        <p className="mt-5 text-lg leading-8 text-slate-300">
          Production should connect this form to Stripe, Shopify, WooCommerce,
          or an invoice workflow before final Merchant Center review.
        </p>
      </div>
      <div className="grid gap-8 lg:grid-cols-[1fr_420px]">
        ...
        >
          ...
          <label className="grid gap-2 text-sm font-bold text-slate-300">
            ...
            <textarea
              name="notes"
              className="min-h-32 rounded-2xl border border-white/10 bg-slate-950 p-4 text-white"
            />
          </label>
          <p className="text-xs text-slate-400">
            By submitting, customer acknowledges the Terms, Refund Policy, and
        ...
        <aside className="gem-panel h-fit rounded-[2rem] p-8">
          ...
          {selected ? (
            <div className="mt-6 rounded-3xl border border-white/10 bg-slate-950/60 p-5">
              <p className="font-black text-white">{selected.title}</p>
              <p className="mt-2 text-sm text-slate-400">SKU {selected.sku}</p>
