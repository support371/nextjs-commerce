      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-4 py-20 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-28">
        <div>
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
            {company.name} packages endpoint posture, phishing readiness,
            compliance evidence, and threat visibility into annual prepaid
            software subscriptions that are clear to buy, deploy, and review.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/store" className="gem-btn-primary">
              Browse software store
            </Link>
            <Link href="/merchant-feed.txt" className="gem-btn-secondary">
              View Merchant feed
            </Link>
          </div>
        ...
        <aside className="gem-panel rounded-[2rem] p-6">
          ...
          {[
            ...
            <div
              key={item}
              className="mt-3 flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm text-slate-300"
            >
              <span>{item}</span>
        ...
        <div className="grid gap-5 md:grid-cols-4">
          {capabilities.map((item) => (
            <article key={item.title} className="gem-panel rounded-3xl p-6">
              <div className="mb-4 grid h-11 w-11 place-items-center rounded-2xl border border-emerald-300/30 bg-emerald-300/10 text-emerald-100">
                ✓
              </div>
        ...
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <h2 className="mt-4 text-4xl font-black tracking-[-0.05em] text-white">
              Cybersecurity software catalog.
            </h2>
          </div>
          <Link href="/store" className="gem-btn-secondary">
            View all products
          </Link>
        </div>
          ...
          {products.slice(0, 3).map((product) => (
            ...
            >
              <div className="relative min-h-56 overflow-hidden bg-slate-900 p-6">
                ...
                <h3 className="relative mt-10 text-2xl font-black tracking-tight text-white">
                  {product.shortTitle}
                </h3>
        ...
        <div className="gem-panel grid gap-8 rounded-[2rem] p-8 lg:grid-cols-[1fr_360px] lg:p-10">
          <div>
            ...
            <h2 className="mt-5 text-4xl font-black tracking-[-0.05em] text-white">
              Ready for domain, checkout, and Merchant Center activation.
            </h2>
            <p className="mt-4 text-slate-300">
              Deploy this storefront to {company.domain}, connect payment or
              invoice processing, confirm policy pages, then submit the live
              feed URL in Google Merchant Center.
            </p>
          </div>
          <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-6">
            ...
            <Link href="/contact" className="gem-btn-primary mt-6 w-full">
              Contact GEM
            </Link>
          </div>
        </div>
